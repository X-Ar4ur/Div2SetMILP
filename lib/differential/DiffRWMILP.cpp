...
DiffRWMILP::DiffRWMILP(const vector<ProcedureHPtr> &procedureHs, std::string target)
...
void DiffRWMILP::evaluation() {
    this->targetNdiff = int(ceil(this->currentRound * ((this->keySize / this->sboxMDP) / this->totalRoundNum))) - 1;
...
void DiffRWMILP::programGenModel() {
    ...
    for (const auto& proc : this->procedureHs) {
        if (proc->getName() == "main") {
            for (const auto & i : proc->getParameters().at(0)) {
                this->tanNameMxIndex["main_" + i->getNodeName()] = this->ACounter;
                this->ACounter++;
            }
            ...
            for (int idx = 0; idx < proc->getBlock().size(); ++idx) {
                ...
                if (ele->getLhs()->getNodeType() == UINT and ele->getNodeName() == "push") {
                    ...
                    // 用于寻找调用key schedule的call，然后来决定本次的key schedule call已经结束
                    bool callFindFlag = false;
                    while (true) {
                        ...
                        if (ele->getOp() == ASTNode::CALL) {
                            ...
                            if (!callFindFlag)
                                callFindFlag = true;
                        } else if (!callFindFlag and ele->getNodeName() == "kschd_push") {
                            kschdInput.push_back(ele->getLhs());
                        } else if (callFindFlag and ele->getOp() != ASTNode::CALL) {
                            idx--;
                            ele = proc->getBlock().at(idx);
                    ...
                    // debug
                    /*std::cout << " print all kschd input : " << std::endl;
                    for (const auto& i : kschdInput) {
                        std::cout << i->getNodeName() << std::endl;
                    }*/
                }
...
void DiffRWMILP::roundFunctionGenModel(const ProcedureHPtr &procedureH, std::vector<ThreeAddressNodePtr> inputKey) {
...
void DiffRWMILP::keyScheduleGenModel(const ProcedureHPtr &procedureH, std::vector<ThreeAddressNodePtr> input,
                                     std::vector<ThreeAddressNodePtr> output) {
    ...
    for (const auto & i : procedureH->getParameters().at(0)) {
        if (this->tanNameMxIndex.count("main_" + input[idx]->getNodeName()) != 0) {
            ...
            //std::cout << i->getNodeName() << " : " << "main_" << input[idx]->getNodeName() << " : " << this->tanNameMxIndex["main_" + input[idx]->getNodeName()] << std::endl;
    ...
    for (int i = 0; i < procedureH->getBlock().size(); ++i) {
        ...
        if (ele->getOp() == ASTNode::XOR) {
            ...
            std::cout << "The word-wise MILP should not have TOUINT operation !" << std::endl;
            assert(false);
        } else if (ele->getOp() == ASTNode::SYMBOLINDEX) {}
        else
            assert(false);
        ...
        if (this->sboxFindFlag) {
            this->sboxFindFlag = false;
            for (const auto& name : this->kschdNameMxIndex) {
                if (name.first == ele->getNodeName()) {
                    this->sboxIndexSave.push_back(name.second);
                }
            }
    ...
    for (int i = 0; i < output.size(); ++i) {
        ...
        //std::cout << "main_" << output[i]->getNodeName() << " : " << this->kschdRtnIdxSave[i] << std::endl;
    }
    this->kschdRtnIdxSave.clear();

}

void DiffRWMILP::XORGenModel(const ThreeAddressNodePtr &left, const ThreeAddressNodePtr &right,
                             const ThreeAddressNodePtr &result, bool ifKschdCall) {
    int inputIdx1 = 0, inputIdx2 = 0;
    if (ifKschdCall) {
        for (const auto &pair: kschdNameMxIndex) {
    ...
    for (auto item : inputIdx) {
        if (item == 0) {
            if (ifKschdCall) {
                item = this->ACounter;
                this->kschdNameMxIndex[left->getNodeName()] = item;
                this->ACounter++;
            } else {
                if (!this->rtnIdxSave.empty()) {
...
void DiffRWMILP::ANDandORGenModel(const ThreeAddressNodePtr &left, const ThreeAddressNodePtr &right,
                                  const ThreeAddressNodePtr &result, bool ifKschdCall) {
    int inputIdx1 = 0, inputIdx2 = 0;
    if (ifKschdCall) {
        for (const auto &pair: kschdNameMxIndex) {
...
void DiffRWMILP::PboxGenModel(const ThreeAddressNodePtr &pbox, const ThreeAddressNodePtr &input,
                              const ThreeAddressNodePtr &output, bool ifKschdCall) {
    std::vector<int> pboxValue = this->Box[pbox->getNodeName()];
    std::vector<int> inputIdx = extIdxFromTOUINTorBOXINDEX(input, ifKschdCall);
...
void DiffRWMILP::linearPermuteGenModel(const ThreeAddressNodePtr &pbox, const ThreeAddressNodePtr &input,
...
void DiffRWMILP::AddGenModel(vector<ThreeAddressNodePtr> &input1, vector<ThreeAddressNodePtr> &input2,
                             const vector<ThreeAddressNodePtr> &output) {
...
bool DiffRWMILP::isConstant(ThreeAddressNodePtr threeAddressNodePtr) {
