...
LinearSBMILP::LinearSBMILP(std::vector<ProcedureHPtr> procedureHs, std::string target, std::string mode, int redMode)
...
void LinearSBMILP::cryptanalysis() {
    programGenModel();
...
void LinearSBMILP::programGenModel() {
                    ...
                    if (ele->getOp() == ASTNode::CALL) {
                        ...
                        for (const auto& tproc : this->procedureHs) {
                            if (tproc->getName() == roundFuncId) {
                                ...
                                roundFunctionThreeForkTraversal(tproc);
...
void LinearSBMILP::roundFunctionThreeForkTraversal(const ProcedureHPtr &procedureH) {
    this->tanThreeForkMap.clear();

    for (int i = 0; i < this->rtnIdxSave.size(); ++i) {
        this->tanThreeForkMap[procedureH->getParameters().at(2).at(i)->getNodeName()] = 0;
    }
    ...
    for (int i = 0; i < procedureH->getBlock().size(); ++i) {
        ...
        if (ele->getOp() == ASTNode::XOR or ele->getOp() == ASTNode::AND or ele->getOp() == ASTNode::OR) {
            functionCallFlag = true;
            this->tanThreeForkMap[ele->getLhs()->getNodeName()]++;
            this->tanThreeForkMap[ele->getRhs()->getNodeName()]++;
            this->tanThreeForkMap[ele->getNodeName()] = 0;
        } else if (ele->getOp() == ASTNode::BOXOP) {
            functionCallFlag = true;
            // 处理input
            ThreeAddressNodePtr left;
            if (ele->getRhs()->getOp() == ASTNode::TOUINT) {
                left = ele->getRhs()->getLhs();
            } else if (ele->getRhs()->getOp() == ASTNode::BOXINDEX) {
                left = ele->getRhs();
            }
            while (left->getOp() == ASTNode::BOXINDEX) {
                this->tanThreeForkMap[left->getLhs()->getNodeName()]++;
                left = left->getRhs();
            }
            this->tanThreeForkMap[left->getNodeName()]++;
            // sbox 和 pbox 的output处理是不一样的，因为对应的size获取方式不一样
            if (ele->getLhs()->getNodeName().substr(0, 4) == "sbox") {
                // 处理output
                for (int j = 0; i < this->sboxOutputSize[ele->getLhs()->getNodeName()]; ++j) {
                    this->tanThreeForkMap[ele->getNodeName() + "_$B$_" + std::to_string(j)] = 0;
                }
            } else if (ele->getLhs()->getNodeName().substr(0, 4) == "pbox") {
                for (int j = 0; i < this->Box[ele->getLhs()->getNodeName()].size(); ++j) {
                    this->tanThreeForkMap[ele->getNodeName() + "_$B$_" + std::to_string(j)] = 0;
                }
            }
            ...
            for (int j = 0; j < input1.size(); ++j) {
                this->tanThreeForkMap[input1[j]->getNodeName()]++;
                this->tanThreeForkMap[input2[j]->getNodeName()]++;
                this->tanThreeForkMap[output[j]->getNodeName()] = 0;
            }
            ...
            for (int j = 0; j < input.size(); ++j) {
                this->tanThreeForkMap[input[j]->getNodeName()]++;
                this->tanThreeForkMap[output[j]->getNodeName()] = 0;
            }
            ...
            if (left->getOp() == ASTNode::FFTIMES) {
                if (left->getLhs()->getNodeName().substr(0, 5) == "pboxm") {
                    ...
                    if (left->getRhs()->getOp() == ASTNode::BOXINDEX) {
                        ThreeAddressNodePtr left = left->getRhs();
                        while (left->getOp() == ASTNode::BOXINDEX) {
                            this->tanThreeForkMap[left->getLhs()->getNodeName()]++;
                            left = left->getRhs();
                        }
            ...
            // 被touint连接后的每个bit对应的three-fork记录应该和连接前的保持一致
            if (ele->getLhs()->getOp() == ASTNode::BOXINDEX) {
                ...
                for (int j = 0; j < size; ++j) {
                    this->tanThreeForkMap[ele->getNodeName() + "_$B$_" + std::to_string(j)] = this->tanThreeForkMap[inputTan[j]->getNodeName()];
                }
            }
        } else
            assert(false);
        ...
        if (!functionCallFlag) {
            ...
            if (ele->getLhs()->getNodeType() == NodeType::ARRAY and ele->getOp() == ASTNode::SYMBOLINDEX) {
                ...
                for (const auto &pair: this->tanThreeForkMap) {
                    if (finderName == pair.first)
                        this->tanThreeForkMap[ele->getNodeName()] = pair.second;
                }
            }
    ...
    for (const auto& rtn : procedureH->getReturns()) {
        for (const auto &pair: this->tanThreeForkMap) {
            if (rtn->getNodeName() == pair.first) {
                this->tanThreeForkMap[pair.first]++;
            }
        }
    }
...
void LinearSBMILP::roundFunctionGenModel(const ProcedureHPtr &procedureH) {
    ...
    for (int i = 0; i < procedureH->getBlock().size(); ++i) {
        ...
        // three-fork
        if (this->tanThreeForkMap[ele->getLhs()->getNodeName()] == 2) {
            ThreeForkGenModel(ele->getLhs(), false);
        }
        else if (this->tanThreeForkMap[ele->getRhs()->getNodeName()] == 2) {
            ThreeForkGenModel(ele->getRhs(), false);
        }
        else if (ele->getOp() == ASTNode::XOR) {
...
void LinearSBMILP::sboxFunctionGenModel(const ProcedureHPtr &procedureH, std::vector<ThreeAddressNodePtr> input, std::vector<ThreeAddressNodePtr> output) {
    ...
    for (int i = 0; i < procedureH->getBlock().size(); ++i) {
        ...
        // three-fork
        if (this->tanThreeForkMap[ele->getLhs()->getNodeName()] == 2) {
            ThreeForkGenModel(ele->getLhs(), true);
        }
        else if (this->tanThreeForkMap[ele->getRhs()->getNodeName()] == 2) {
            ThreeForkGenModel(ele->getRhs(), true);
        }
        else if (ele->getOp() == ASTNode::XOR) {
            // XOR with keys or constants does not affect differential propagation
...
void LinearSBMILP::ThreeForkGenModel(const ThreeAddressNodePtr& input, bool ifSboxFuncCall) {
    int inputIdx = 0;
    if (ifSboxFuncCall) {
        for (const auto& pair : this->sboxNameMxIndex) {
            if (pair.first == input->getNodeName())
                inputIdx = pair.second;
        }
    } else {
        for (const auto& pair : this->tanNameMxIndex) {
            if (pair.first == input->getNodeName())
    ...
    if (ifSboxFuncCall) {
        if (input->getOp() == ASTNode::SYMBOLINDEX and input->getLhs()->getNodeType() != NodeType::UINT1) {
            for (const auto& pair : this->sboxNameMxIndex) {
                if (pair.first == input->getLhs()->getNodeName() + "_$B$_" + input->getRhs()->getNodeName()) {
                    inputIdx = pair.second;
                    this->sboxNameMxIndex[input->getNodeName()] = inputIdx;
                }
            }
        }
    } else {
        if (input->getOp() == ASTNode::SYMBOLINDEX and input->getLhs()->getNodeType() != NodeType::UINT1) {
            for (const auto& pair : this->tanNameMxIndex) {
                if (pair.first == input->getLhs()->getNodeName() + "_$B$_" + input->getRhs()->getNodeName()) {
                    inputIdx = pair.second;
                    this->tanNameMxIndex[input->getNodeName()] = inputIdx;
                }
            }
        }
    }
    ...
    if (ifSboxFuncCall) {
        if (inputIdx == 0) {
            if (!this->sboxRtnIdxSave.empty()) {
                inputIdx = this->sboxRtnIdxSave.front();
                this->sboxRtnIdxSave.erase(this->sboxRtnIdxSave.cbegin());
                this->sboxNameMxIndex[input->getNodeName()] = inputIdx;
            } else {
                inputIdx = this->xCounter;
                this->sboxNameMxIndex[input->getNodeName()] = inputIdx;
                this->xCounter++;
            }
        }
        ...
        if (inputIdx == 0) {
            if (!this->rtnIdxSave.empty()) {
                inputIdx = this->rtnIdxSave.front();
    ...
    int outputIdx1 = this->xCounter;
    this->xCounter++;
    int outputIdx2 = this->xCounter;
    this->xCounter++;

    //std::vector<std::string> outputIdxStr;
    //outputIdxStr.push_back(std::to_string(outputIdx1));
    //outputIdxStr.push_back(std::to_string(outputIdx2));
    // 该input对应的threeAddressNode的后续约束构建时，先使用outputIdx1
    this->tanNameMxIndex[input->getNodeName()] = outputIdx1;
    this->tanThreeForkNameMxIndex[input->getNodeName()] = outputIdx2;

    LinearMILPcons::bThreeForkC(this->modelPath, inputIdx, outputIdx1, outputIdx2, this->dCounter);
}


void LinearSBMILP::XORGenModel(const ThreeAddressNodePtr &left, const ThreeAddressNodePtr &right,
    ...
    this->xCounter++;
    LinearMILPcons::bXorC(this->modelPath, inputIdx1, inputIdx2, outputIdx);


    // 若某一个input是three-fork的输出，则更改对应threeAddressNode的map
    if (this->tanThreeForkMap.find(left->getNodeName())->second == 2) {
        this->tanNameMxIndex[left->getNodeName()] = this->tanThreeForkNameMxIndex[left->getNodeName()];
    }
    if (this->tanThreeForkMap.find(right->getNodeName())->second == 2) {
        this->tanNameMxIndex[right->getNodeName()] = this->tanThreeForkNameMxIndex[right->getNodeName()];
    }
}


void LinearSBMILP::ANDandORGenModel(const ThreeAddressNodePtr &left, const ThreeAddressNodePtr &right,
                                  const ThreeAddressNodePtr &result, bool ifSboxFuncCall) {
    int inputIdx1 = 0, inputIdx2 = 0;
    if (ifSboxFuncCall) {
        for (const auto& pair : this->sboxNameMxIndex) {
    ...
    this->xCounter++;
    if (mode == "AS")
        LinearMILPcons::bAndOrC(this->modelPath, inputIdx1, inputIdx2, outputIdx);
    else if (mode == "DC")
        LinearMILPcons::dAndOrC(this->modelPath, inputIdx1, inputIdx2, outputIdx, this->PCounter);
    else
        assert(false);
...
void LinearSBMILP::SboxGenModel(const ThreeAddressNodePtr &sbox, const ThreeAddressNodePtr &input,
    ...
    if (this->mode == "AS") {
        switch (this->sboxConSel) {
            case 1:
                LinearMILPcons::bSboxC1(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
                                      this->branchNumMin[sbox->getNodeName()], this->dCounter, this->ACounter, sboxIfInjective[sbox->getNodeName()]);
                break;
            case 2:
                LinearMILPcons::bSboxC2(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
                                      this->ACounter, sboxIfInjective[sbox->getNodeName()]);
                break;
            case 3:
                LinearMILPcons::bSboxC3(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
                                      this->ACounter);
                break;
            default:
                LinearMILPcons::bSboxC2(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
        ...
        switch (this->sboxConSel) {
            case 1:
                LinearMILPcons::dSboxC1(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
                ...
                break;
            case 2:
                LinearMILPcons::dSboxC2(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
                                      this->PCounter, sboxIfInjective[sbox->getNodeName()], this->sboxExtWeighted.size());
                break;
            case 3:
                LinearMILPcons::dSboxC3(this->modelPath, inputIdx, outputIdx, this->sboxIneqs[sbox->getNodeName()],
...
void LinearSBMILP::PboxGenModel(const ThreeAddressNodePtr &pbox, const ThreeAddressNodePtr &input,
                              const ThreeAddressNodePtr &output) {
...
void LinearSBMILP::MatrixVectorGenModel(const ThreeAddressNodePtr &pboxm, const ThreeAddressNodePtr &input,
    ...
    if (eleSize != 1) {
        ...
        for (int i = 0; i < MoutIdxIntIdxes.size(); ++i) {
            if (matrixConSel == 1 or matrixConSel == 2) {
                LinearMILPcons::bMatrixEntryC12(this->modelPath, itemMidx->second, itemMidx->first, itemMove->second, moduleVec,
                                              this->xCounter, this->yCounter, this->fCounter, this->dCounter, this->matrixConSel);
            } else if (matrixConSel == 2) {
                LinearMILPcons::bMatrixEntryC3(this->modelPath, itemMidx->second, itemMidx->first, itemMove->second, moduleVec,
...
void LinearSBMILP::multiXORGenModel12(std::vector<ThreeAddressNodePtr> input, ThreeAddressNodePtr result) {
    ...
    /*switch (matrixConSel) {
        case 1:
            LinearMILPcons::bXorC1(this->modelPath, inputIdx1, inputIdx2, outputIdx, this->dCounter);
            break;
        case 2:
            LinearMILPcons::bXorC2(this->modelPath, inputIdx1, inputIdx2, outputIdx);
            break;
        default:
            LinearMILPcons::bXorC2(this->modelPath, inputIdx1, inputIdx2, outputIdx);
    }*/
...
void LinearSBMILP::multiXORGenModel3(std::vector<ThreeAddressNodePtr> input, ThreeAddressNodePtr result) {
    std::vector<int> inputIdx;
    for (auto ele : input) {
        bool finder = false;
        for (const auto& pair : this->tanNameMxIndex) {
    ...
    this->xCounter++;
    this->tanNameMxIndex[result->getNodeName()] = outputIdx;
    LinearMILPcons::bNXorC3(this->modelPath, inputIdx, outputIdx, this->yCounter);
}


void LinearSBMILP::ADDandMINUSGenModel(vector<ThreeAddressNodePtr> &input1, vector<ThreeAddressNodePtr> &input2,
    ...
    if (this->mode == "AS") {
        // Add
        if (AddOrMinus == 1)
            LinearMILPcons::bAddC(this->modelPath, inputIdx1, inputIdx2, outputIdx, this->ARXineqs, dCounter, PCounter);
            // Minus
        else if (AddOrMinus == 2)
            // a - b = c   ->   a = b + c   ->   b + c = a
            LinearMILPcons::bAddC(this->modelPath, outputIdx, inputIdx2, inputIdx1, this->ARXineqs, dCounter, PCounter);
        else
            assert(false);
    } else if (this->mode == "DC") {
        // Add
        if (AddOrMinus == 1)
            LinearMILPcons::dAddC(this->modelPath, inputIdx1, inputIdx2, outputIdx, this->ARXineqs, dCounter, PCounter);
            // Minus
        else if (AddOrMinus == 2)
            // a - b = c   ->   a = b + c   ->   b + c = a
            LinearMILPcons::dAddC(this->modelPath, outputIdx, inputIdx2, inputIdx1, this->ARXineqs, dCounter, PCounter);
        else
            assert(false);
    } else
        assert(false);
