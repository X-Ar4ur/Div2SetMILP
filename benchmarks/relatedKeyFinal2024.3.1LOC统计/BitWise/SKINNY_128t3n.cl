...
k_schd uint1[384] key_schedule(uint1[384] key) {
    uint1[128] tk1 = pt<View(Key, 0, 127)>;
    uint1[128] tk2 = pt<View(Key, 128, 255)>;
    uint1[128] tk3 = pt<View(Key, 256, 384)>;
    uint1[64] tktt = View(tk2, 0, 63) >>> 8;
    for (i from 0 to 7) {
        tktt[i] = tktt[i] ^ tktt[i + 48];
    }
    for (i from 0 to 63) {
        tk2[i] = tktt[i];
    }
    uint1[64] tktt2 = View(tk3, 0, 63) <<< 8;
    for (i from 56 to 63) {
        tktt[i] = tktt[i] ^ tktt[i - 16];
    }
    for (i from 0 to 63) {
        tk3[i] = tktt[i];
    }
...
r_fn uint1[128] round_function(uint8 r, uint1[384] key, uint1[128] input) {
    uint1[128] s_out;
    for (i from 0 to 15) {
        uint1[8] temp = View(input, i*4, i*4+7);
        uint8 sbox_in = touint(temp[7], temp[6], temp[5], temp[4], temp[3], temp[2], temp[1], temp[0]);
        uint8 sbox_out = s<sbox_in>;
        s_out[i * 4 + 0] = sbox_out[7];
        s_out[i * 4 + 1] = sbox_out[6];
        s_out[i * 4 + 2] = sbox_out[5];
        s_out[i * 4 + 3] = sbox_out[4];
        s_out[i * 4 + 4] = sbox_out[3];
        s_out[i * 4 + 5] = sbox_out[2];
        s_out[i * 4 + 6] = sbox_out[1];
        s_out[i * 4 + 7] = sbox_out[0];
    }
    uint6 c = rc[r];
    ...
    for (i from 0 to 7) {
        ...
        s_out[i + 32] = s_out[i + 32] ^ c0[7-i];
        s_out[i + 64] = s_out[i + 64] ^ c0[7-i];
    }
    uint1[128] t_out = s_out;
    for (i from 0 to 127) {
        t_out[i] = s_out[i] ^ key[i] ^ key[i+128] ^ key[i+256];
    }
    uint1[128] p_out = p<t_out>;
    uint1[128] m_out;
    for (i from 0 to 3) {
        uint1[32] mtemp = View(input, i*32, i*32+31);
        uint8[4] mmt;
        for (j from 0 to 3) {
            mmt[i] = touint(mtemp[j*8+7], mtemp[j*8+6], mtemp[j*8+5], mtemp[j*8+4], mtemp[j*8+3], mtemp[j*8+2], mtemp[j*8+1], mtemp[j*8]);
        }
        uint8[4] sm_out = M * mtemp;
        for (j from 0 to 3) {
            for (k from 0 to 7) {
                m_out[i*4+j*8+k] = sm_out[7-k];
            }
        }
    }
    return m_out;
}
fn uint1[128] enc(uint1[384] key, uint1[128] r_plaintext){
    for (i from 1 to 40) {
