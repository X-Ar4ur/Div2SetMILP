...
k_schd uint1[1152] key_schedule(uint1[80] key) {
    ...
    for (j from 1 to 35) {
        ...
        for (i from 0 to 3) {
            rtn[i + counter] = wk1[i];
            rtn[i + 4 + counter] = wk3[i];
            rtn[i + 8 + counter] = wk4[i];
            rtn[i + 12 + counter] = wk6[i];
            rtn[i + 16 + counter] = wk13[i];
            rtn[i + 20 + counter] = wk14[i];
            rtn[i + 24 + counter] = wk15[i];
            rtn[i + 28 + counter] = wk16[i];
        }
    }
    return rtn;
}
...
fn uint1[64] enc(uint1[80] key, uint1[64] r_plaintext){
    uint1[1152] sub_keys = key_schedule(key);
    for (i from 1 to 35) {
        r_plaintext = round_function1(i, View(sub_keys, (i - 1) * 32, i * 32 - 1), r_plaintext);
    }
    r_plaintext = round_function2(36, View(sub_keys, 1120, 1151), r_plaintext);
    return r_plaintext;
}