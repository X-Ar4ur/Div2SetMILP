...
k_schd uint1[64] key_schedule(uint1[64] key) {
    uint1[64] key_rhs = key >>> 15;
    uint4 sbox_in1 = touint(key_rhs[0], key_rhs[1], key_rhs[2], key_rhs[3]);
    uint4 sbox_out1 = s<sbox_in1>;
    uint4 sbox_in2 = touint(key_rhs[4], key_rhs[5], key_rhs[6], key_rhs[7]);
    uint4 sbox_out2 = s<sbox_in1>;
    key_rhs[0] = sbox_out1[0];
    key_rhs[1] = sbox_out1[1];
    key_rhs[2] = sbox_out1[2];
    key_rhs[3] = sbox_out1[3];
    key_rhs[4] = sbox_out2[0];
    key_rhs[5] = sbox_out2[1];
    key_rhs[6] = sbox_out2[2];
    key_rhs[7] = sbox_out2[3];
    return key_rhs;
}
