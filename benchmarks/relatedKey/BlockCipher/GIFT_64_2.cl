...
k_schd uint1[128] key_schedule(uint1[128] key) {
    uint1[16] k7 = View(key, 0, 15);
    uint1[16] k6 = View(key, 16, 31);
    uint1[16] k5 = View(key, 32, 47);
    uint1[16] k4 = View(key, 48, 63);
    uint1[16] k3 = View(key, 64, 79);
    uint1[16] k2 = View(key, 80, 95);
    uint1[16] k1 = View(key, 96, 111);
    uint1[16] k0 = View(key, 112, 127);
    k1 = k1 >>> 2;
    k0 = k0 >>> 12;
    uint1[128] rtn;
    for (i from 0 to 15) {
...
r_fn uint1[64] round_function(uint8 r, uint1[32] key, uint1[64] input) {
	...
	for (i from 0 to 15) {
	    p_out[4 * i + 2] = p_out[4 * i + 2] ^ key[i + 16];
	    p_out[4 * i + 3] = p_out[4 * i + 3] ^ key[i];
	}
    # p_out[63] = p_out[63] ^ 1;
