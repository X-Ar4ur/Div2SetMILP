...
k_schd uint8[24] key_schedule(uint8[24] key) {
    uint8[4] k0 = View(key, 0, 3);
    uint8[4] k1 = View(key, 4, 7);
    uint8[4] k2 = View(key, 8, 11);
    uint8[4] k3 = View(key, 12, 15);
    uint8[4] k4 = View(key, 16, 19);
    uint8[4] k5 = View(key, 20, 23);
...
fn uint8[16] enc(uint8[24] key, uint8[16] r_plaintext){
    r_plaintext = round_function1(1, View(key, 0, 15), r_plaintext);
    uint8[16] subkey;
    for (j from 0 to 3) {
        for (i from 0 to 7) {
            subkey[i] = key[i+16];
        ...
        r_plaintext = round_function2(j*3+2, subkey, r_plaintext);
        for (i from 0 to 15) {
            subkey[i] = key[i+8];
        }
        r_plaintext = round_function2(j*3+3, subkey, r_plaintext);
        key = key_schedule(key);
        for (i from 0 to 15) {
            subkey[i] = key[i+8];
        }
        r_plaintext = round_function2(j*3+4, subkey, r_plaintext);
    }
    for (i from 0 to 7) {
        subkey[i] = key[i+16];
    }
    ...
    r_plaintext = round_function3(14, subkey, r_plaintext);
    return r_plaintext;
}