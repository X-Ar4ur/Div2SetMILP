...
k_schd uint8[16] key_schedule(uint8[16] key) {
...
fn uint8[16] enc(uint8[16] key, uint8[16] r_plaintext){
    r_plaintext = round_function1(1, key, r_plaintext);
    for (i from 2 to 9) {
        key = key_schedule(key);
        r_plaintext = round_function2(10, key, r_plaintext);
    }
    key = key_schedule(key);
    r_plaintext = round_function3(11, key, r_plaintext);
    return r_plaintext;
}