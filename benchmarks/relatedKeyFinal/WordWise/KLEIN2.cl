...
k_schd uint4[16] key_schedule(uint4[16] key, uint roundCounter) {
    uint4[8] a = View(key, 0, 7);
    uint4[8] b = View(key, 8, 15);
    a = a <<< 2;
    b = b <<< 2;
    uint4[8] a1 = b;
    a1[3] = a1[3] ^ roundCounter;
    uint4[8] b1 = a ^ b;
    uint4[16] rtn;
    for (i from 0 to 7) {
        rtn[i] = a1[i];
    ...
    uint4 sbox_in1 = rtn[10];
    uint4 sbox_in2 = rtn[11];
    uint4 sbox_in3 = rtn[12];
    uint4 sbox_in4 = rtn[13];
    uint4 sbox_out1 = s<sbox_in1>;
    uint4 sbox_out2 = s<sbox_in2>;
    uint4 sbox_out3 = s<sbox_in3>;
    uint4 sbox_out4 = s<sbox_in4>;
    rtn[10] = sbox_out1;
    rtn[11] = sbox_out2;
    rtn[12] = sbox_out3;
    rtn[13] = sbox_out4;
...
fn uint4[16] enc(uint4[16] key, uint4[16] r_plaintext){
    for (i from 1 to 12) {
