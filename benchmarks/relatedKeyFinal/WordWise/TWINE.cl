...
k_schd uint4[8] key_schedule1(uint4[20] key) {
    uint4[8] rtn;
    rtn[0] = key[1];
    rtn[1] = key[3];
    rtn[2] = key[4];
...
k_schd uint4[28] key_schedule2(uint4[20] key) {
    uint4[28] rtn;
...
fn uint4[16] enc(uint4[20] key, uint4[16] r_plaintext){
    uint4[8] sub_keys = key_schedule1(key);
    r_plaintext = round_function1(i, sub_keys, r_plaintext);
    uint4[28] round_keys;
    round_keys = key_schedule2(key);
    r_plaintext = round_function1(i, View(round_keys, 0, 7), r_plaintext);
    for (i from 3 to 35) {
        round_keys = key_schedule2(View(round_keys, 8, 27));
        r_plaintext = round_function1(i, View(round_keys, 0, 7), r_plaintext);
    }
    round_keys = key_schedule2(View(round_keys, 8, 27));
    r_plaintext = round_function2(36, View(round_keys, 0, 7), r_plaintext);
    return r_plaintext;
}