...
#ifndef EASYBC_DIFFRBMILP_H
#define EASYBC_DIFFRBMILP_H
...
/*
 * Related-Key Bit-wise MILP for differential cryptanalysis
 *
 * Related-Key的建模应该和Single-Key的建模区别在于key schedule的建模与否
 * 而我们在调用key schedule的地方分别对他们进行如round function类似的建模即可
 * 根据key schedule的调用位置，我们分两种情况：
 *      1. main函数中调用round function之前调用key schedule :
 *
 *         当main函数的ele名字为 "kschd_push" 时，我们就找到了key shcedule调用
 *
 *
 *
 *
 *
 *      2. round function中调用key schedule
 *
 *
 * 有一个问题我们需要思考，对key schedule进行建模时，应该和sboxFunction一样，
 * 即每个key schedule都独立建模，这里我们暂时考虑类似于sbox function的建模，
 * 由key schedule的procedure和input，output，确定出对应的model
 *
 *
 * */
...
private:
    ...
    // **************** analysis structure ****************
    std::string pathPrefix = std::string(DPATH) + "differential/relatedKey/BitWiseMILP/";
    ...
    // 当在key schedule内调用各种操作的建模函数时，用于存储对应于key schedule的各个对应的xCounter
    map<std::string, int> kschdNameMxIndex;
    // 当在key schedule内调用各种操作的建模函数时，用于存储对应于key schedule的return值的xCounter
    std::vector<int> kschdRtnIdxSave;
    std::vector<int> kschdACounterSave; // 用于保存key schedule中使用的ACounter，这样可以在obj function排除掉这部分ACounter
...
public:
    DiffRBMILP(std::vector<ProcedureHPtr> procedureHs, std::string target, std::string mode, int redMode);
    ...
    void roundFunctionGenModel(const ProcedureHPtr& procedureH, std::vector<ThreeAddressNodePtr> inputKey);

    void keyScheduleGenModel(const ProcedureHPtr& procedureH, std::vector<ThreeAddressNodePtr> input, std::vector<ThreeAddressNodePtr> output);
    ...
    void XORGenModel(const ThreeAddressNodePtr& left, const ThreeAddressNodePtr& right, const ThreeAddressNodePtr& result, bool ifSboxFuncCall, bool ifKschdCall);

    void ANDandORGenModel(const ThreeAddressNodePtr& left, const ThreeAddressNodePtr& right, const ThreeAddressNodePtr& result, bool ifSboxFuncCall, bool ifKschdCall);

    void SboxGenModel(const ThreeAddressNodePtr& sbox, const ThreeAddressNodePtr& input, const ThreeAddressNodePtr& output, bool ifKschdCall);

    void PboxGenModel(const ThreeAddressNodePtr& pbox, const ThreeAddressNodePtr& input, const ThreeAddressNodePtr& output, bool ifKschdCall);
    ...
    void ADDandMINUSGenModel(std::vector<ThreeAddressNodePtr>& input1, std::vector<ThreeAddressNodePtr>& input2,
                             const std::vector<ThreeAddressNodePtr>& output, int AddOrMinus, bool ifSboxFuncCall, bool ifKschdCall);
    ...
    void preprocess(std::map<std::string, std::vector<int>> pboxM, std::map<std::string, std::vector<int>> Ffm) {
        ...
        while (iterator != this->Box.end()) {
            if (iterator->first.substr(0, 4) == "sbox") {
                ...
                SboxM sboxM(sboxName, iterator->second, "differential", this->mode);
    ...
    std::vector<int> extIdxFromTOUINTorBOXINDEX(const ThreeAddressNodePtr& input, bool ifSboxFuncCall, bool ifKschdCall) {
        ...
        if (ifSboxFuncCall) {
            if (input->getOp() == ASTNode::TOUINT) {
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    ...
                    if (sboxNameMxIndex.count(left->getLhs()->getNodeName()) != 0) {
                        extIdx.push_back(sboxNameMxIndex.find(left->getLhs()->getNodeName())->second);
                        left = left->getRhs();
                    } else {
                        sboxNameMxIndex[left->getLhs()->getNodeName()] = xCounter;
                        extIdx.push_back(xCounter);
                        xCounter++;
                        left = left->getRhs();
                    }
                }
                if (sboxNameMxIndex.count(left->getNodeName()) != 0) {
                    extIdx.push_back(sboxNameMxIndex.find(left->getNodeName())->second);
                } else {
                    sboxNameMxIndex[left->getLhs()->getNodeName()] = xCounter;
                    extIdx.push_back(xCounter);
                    xCounter++;
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    if (sboxNameMxIndex.count(left->getLhs()->getNodeName()) != 0) {
                        ...
                        if (left->getLhs()->getOp() == ASTNode::SYMBOLINDEX and left->getLhs()->getLhs()->getNodeType() != UINT1) {
                            if (sboxNameMxIndex.count(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                       left->getLhs()->getRhs()->getNodeName()) != 0) {
                                extIdx.push_back(sboxNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                       left->getLhs()->getRhs()->getNodeName())->second);
                                sboxNameMxIndex[left->getNodeName()] = sboxNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                                              left->getLhs()->getRhs()->getNodeName())->second;
                                left = left->getRhs();
                            } else
                                assert(false);
                ...
                if (sboxNameMxIndex.count(left->getNodeName()) != 0) {
                    ...
                    sboxNameMxIndex[left->getNodeName()] = xCounter;
                    extIdx.push_back(xCounter);
                    xCounter++;
                }
            } else assert(false);
            ...
            if (input->getOp() == ASTNode::TOUINT) {
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    ...
                    // have been stored in the map of tanNameMxIndex.
                    // If there is, there is no need to repeat the processing and directly end the operation of the function
                    if (kschdNameMxIndex.count(left->getLhs()->getNodeName()) != 0) {
                        extIdx.push_back(kschdNameMxIndex.find(left->getLhs()->getNodeName())->second);
                        left = left->getRhs();
                    } else {
                        kschdNameMxIndex[left->getLhs()->getNodeName()] = xCounter;
                        extIdx.push_back(xCounter);
                        xCounter++;
                        left = left->getRhs();
                    }
                }
                if (kschdNameMxIndex.count(left->getNodeName()) != 0) {
                    extIdx.push_back(kschdNameMxIndex.find(left->getNodeName())->second);
                } else {
                    kschdNameMxIndex[left->getLhs()->getNodeName()] = xCounter;
                    extIdx.push_back(xCounter);
                    xCounter++;
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    ...
                    // 但是其可能是某个uints的某一位，而这个uints已经被map，所以需要寻找该uints的对应某一位的map
                        ...
                        if (left->getLhs()->getOp() == ASTNode::SYMBOLINDEX and left->getLhs()->getLhs()->getNodeType() != UINT1) {
                            if (kschdNameMxIndex.count(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                     left->getLhs()->getRhs()->getNodeName()) != 0) {
                                extIdx.push_back(kschdNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                     left->getLhs()->getRhs()->getNodeName())->second);
                                kschdNameMxIndex[left->getNodeName()] = kschdNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                                          left->getLhs()->getRhs()->getNodeName())->second;
                                left = left->getRhs();
                            } else
                                assert(false);
                ...
                if (kschdNameMxIndex.count(left->getNodeName()) != 0) {
                    ...
                    kschdNameMxIndex[left->getNodeName()] = xCounter;
                    extIdx.push_back(xCounter);
                    xCounter++;
                }
            } else assert(false);
