//
// Created by Septi on 7/12/2023.
//

#ifndef EASYBC_DIFFRWMILP_H
#define EASYBC_DIFFRWMILP_H
...
/*
 * Word-wise MILP used to calculate the minimal number of related-key differential active S-boxes
 * */
...
private:
    ...
    // **************** analysis structure ****************
    std::string pathPrefix = std::string(DPATH) + "differential/relatedKey/WordWiseMILP/";
...
public:
    ...
    DiffRWMILP(const vector<ProcedureHPtr>& procedureHs, std::string target);
    ...
    void XORGenModel(const ThreeAddressNodePtr& left, const ThreeAddressNodePtr& right, const ThreeAddressNodePtr& result, bool ifKschdCall);

    void ANDandORGenModel(const ThreeAddressNodePtr& left, const ThreeAddressNodePtr& right, const ThreeAddressNodePtr& result, bool ifKschdCall);
        ...
        while (iterator != Box.end()) {
            if (iterator->first.substr(0, 4) == "sbox") {
                ...
                if (iterator->second.size() <= 32) {
                    SboxM sboxM(sboxName, iterator->second, "differential", "AS"); // word-wise 时需要的MDP通过AS mode来建模获取
                    // 获取sboxMDP
    ...
    std::vector<int> extIdxFromTOUINTorBOXINDEX(const ThreeAddressNodePtr& input, bool ifKschdCall) {
        ...
        if (ifKschdCall) {
            if (input->getOp() == ASTNode::TOUINT) {
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    ...
                    if (this->kschdNameMxIndex.count(left->getLhs()->getNodeName()) != 0) {
                        extIdx.push_back(this->kschdNameMxIndex.find(left->getLhs()->getNodeName())->second);
                        left = left->getRhs();
                    }
                    else {
                        this->kschdNameMxIndex[left->getLhs()->getNodeName()] = this->ACounter;
                        extIdx.push_back(this->ACounter);
                        this->ACounter++;
                        left = left->getRhs();
                    }
                }
                if (this->kschdNameMxIndex.count(left->getNodeName()) != 0) {
                    extIdx.push_back(this->kschdNameMxIndex.find(left->getNodeName())->second);
                } else {
                    this->kschdNameMxIndex[left->getLhs()->getNodeName()] = this->ACounter;
                    extIdx.push_back(this->ACounter);
                    this->ACounter++;
                ...
                while (left->getOp() == ASTNode::BOXINDEX) {
                    ...
                    // 但是其可能是某个uints的某一位，而这个uints已经被map，所以需要寻找该uints的对应某一位的map
                        ...
                        if (left->getLhs()->getOp() == ASTNode::SYMBOLINDEX and
                            left->getLhs()->getLhs()->getNodeType() != UINT1) {
                            if (this->kschdNameMxIndex.count(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                           left->getLhs()->getRhs()->getNodeName()) != 0) {
                                extIdx.push_back(
                                        this->kschdNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                  left->getLhs()->getRhs()->getNodeName())->second);
                                this->kschdNameMxIndex[left->getNodeName()] = this->kschdNameMxIndex.find(
                                        left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                        left->getLhs()->getRhs()->getNodeName())->second;
                                left = left->getRhs();
                            } else
                                assert(false);
                ...
                if (this->kschdNameMxIndex.count(left->getNodeName()) != 0) {
                    ...
                    this->kschdNameMxIndex[left->getNodeName()] = this->ACounter;
                    extIdx.push_back(this->ACounter);
                    this->ACounter++;
                }
            } else
                    ...
                    if (this->tanNameMxIndex.count(left->getLhs()->getNodeName()) != 0) {
                        ...
                        if (left->getLhs()->getOp() == ASTNode::SYMBOLINDEX and
                            ...
                            if (this->tanNameMxIndex.count(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                           left->getLhs()->getRhs()->getNodeName()) != 0) {
                                extIdx.push_back(
                                        this->tanNameMxIndex.find(left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                                                  left->getLhs()->getRhs()->getNodeName())->second);
                                this->tanNameMxIndex[left->getNodeName()] = this->tanNameMxIndex.find(
                                        left->getLhs()->getLhs()->getNodeName() + "_$B$_" +
                                        left->getLhs()->getRhs()->getNodeName())->second;
                                left = left->getRhs();
                            } else
